...
quota_opts = [
    ...
    cfg.StrOpt('quota_driver',
               default='nova.quota.NestedQuotaDriver',
               help='Driver to use for nested quota checks'),
    ]
...
class NestedQuotaDriver(object):
    """Driver to perform necessary checks to enforce nested quotas and
    obtain quota information for hierarchical project.
        ...
        if user_id:
            ...
            if parent_id:
                db_parent_project_quotas = \
                    db.quota_get_all_by_project(context, parent_id)
                parent_project_allocated_quotas = \
                    db.quota_allocated_get_all_by_project(context, parent_id)
                parent_project_quotas =\
                    self.get_project_quotas(context, resources,
            ...
            for key, value in project_quotas.items():
                ...
                if(parent_id):
                    maximum = parent_project_quotas[key]['remains'] - \
                              parent_project_allocated_quotas.get(key, 0)\
                              + value['limit']
                minimum = \
                    max(int(self._sub_quota_values(value['limit'],
                                                   value['remains'])),
